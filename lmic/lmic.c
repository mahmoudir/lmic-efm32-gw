...
#endif
    {
    	printf("PAMBL_SYMS++++++++++++++++++++++++++ LMIC.txend %ld\n",LMIC.txend);
        LMIC.rxtime = LMIC.txend + delay + (PAMBL_SYMS-MINRX_SYMS)*dr2hsym(LMIC.dndr);
        //printf("LMIC.rxtime %ld = difference %ld\n",LMIC.rxtime,LMIC.rxtime - LMIC.txend);
        LMIC.rxsyms = MINRX_SYMS;
    }
    os_setTimedCallback(&LMIC.osjob, LMIC.rxtime - RX_RAMPUP  - 20000 , func);
}
...
static bit_t processJoinAccept (void) {
	printf("Enter processJoinAccept\n");
...
static void engineUpdate (void) {
    ...
    if( (LMIC.opmode & (OP_JOINING|OP_REJOIN|OP_TXDATA|OP_POLL)) != 0 ) {
        ...
        printf("engineUpdate Cannot yet TX\n");
    	...
    	printf("No TX pending - no scheduled RX\n");
